...
type Range = "24H" | "12H" | "1W" | "1M" | "3M" | "6M" | "1Y";
const RANGES: Range[] = ["24H", "12H", "1W", "1M", "3M", "6M", "1Y"];
...
export default function StockPriceChart({ symbol }: StockPriceChartProps) {
  ...
  const hasData = !isLoading && !isError && points.length > 0;

  const firstPrice = hasData ? points[0].price : 0;
  const lastPrice = hasData ? points.at(-1)!.price : 0;
  ...
  const isIntraday = range === "24H" || range === "12H";
  const chartData = points.map((p) => ({
    ...p,
    label: isIntraday ? formatHourLabel(p.date) : formatDateShort(p.date),
  }));
      ...
      {hasData && (
        <ResponsiveContainer width="100%" height={220}>
          ...
          >
            ...
            />
            <Tooltip
              content={<CustomTooltip isIntraday={isIntraday} />}
